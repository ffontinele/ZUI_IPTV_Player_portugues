...
const wpoly=fs.readFileSync(fs.existsSync(p.join(__dirname,'wpoly.js'))?p.join(__dirname,'wpoly.js'):'/tmp/zui-wpoly.js','utf8');
...
fs.writeFileSync(p.join(A,'zui-mainpoly.js'),fs.readFileSync(fs.existsSync(p.join(__dirname,'mainpoly.js'))?p.join(__dirname,'mainpoly.js'):'/tmp/zui-mainpoly.js','utf8');
